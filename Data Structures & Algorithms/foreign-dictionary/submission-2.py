class Solution:
    def foreignDictionary(self, words: List[str]) -> str:
        #1. initialize each char
        char_set = {c: set() for word in words for c in word}

        #check for prefix and match preceeding characters with
        #d characters that it preceeds 
        for i in range(len(words) - 1):
            ...
            for j in range(min_len):
                if words[i][j] != words[i + 1][j]:
                    char_set[words[i][j]].add(words[i + 1][j])
                    break
